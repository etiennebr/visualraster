...
library("igraph")    # edge and clumps
library("fields")    # Tps for interpolation
...
```{r}
r_focal_nedge = focal(r, fun = mean, na.rm = TRUE, pad = TRUE)
```
...
r_edge = raster::edge(r_clus)
```

```{r echo = FALSE}
ggplot() + vr_geom_raster(r_clus) + vr_geom_text(r_clus)
ggplot() + vr_geom_raster(r_edge) + vr_geom_text(r_edge)
```
...
ggplot() + vr_geom_raster_seq(interpolate(r_volcano, tps, fun=predict.se)) + ggtitle("Standard Error") + gsurvey
```
